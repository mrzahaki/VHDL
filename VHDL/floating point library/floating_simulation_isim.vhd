...
entity floating is
  generic(
	float_typedef :float_type := float_single_precision--float_single_precision-- see DROGRAMMER.std_float
  );
end floating;
...
architecture Behavioral of floating is
    ...
    signal adder_0, adder_1 :  singlePrecision_type := (others=> '0') ;
	signal adder_out :  singlePrecision_type;
	
	
	--fmul inputs
	signal mul_0, mul_1 :  singlePrecision_type:= (others=> '0') ;
	signal mul_out :  singlePrecision_type;
...
begin
	...
	FA1: fmul generic map (float_typedef)  				port map( 	mul_0, mul_1, mul_out);-- see DROGRAMMER.std_float
	  ...
	  -------------------------------------------------------------------------
  -- This process reads the file input_floating_points.txt which is located in the
  -- work document.
  ...
  -- note for example input_floating_points file format is:
